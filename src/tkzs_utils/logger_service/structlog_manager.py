...
def create_custom_logger_class(custom_levels: list[CustomLoggerLevel]):
    ...
    for custom_level in custom_levels:
        def _make_level_method(level_val: int, method_name: str):
            ...
            def level_method(self, event=None, *args, **kwargs):
                ...
                try:
                    ...
                    return None
                return self._logger.log(level_val, *proc_args, **proc_kw)

            level_method.__name__ = mname
            return level_method

        key = custom_level.level_name.lower()
        class_attrs[key] = _make_level_method(
            custom_level.level_value,
            custom_level.level_name,
        )
...
def create_process_chain(process_chain: ProcessChainSettings):
    ...
    shared_processors.append(structlog.processors.TimeStamper(fmt=process_chain.timestamp_format,utc=process_chain.utc)) # 添加 timestamp 字段 (默认格式ISO 8601)
...
def update_structlog_configuration(structlog_settings: StructLogSettings):
    ...
    process_chain_processors.append(structlog.dev.ConsoleRenderer(
            colors=True,
        ))
    structlog.configure(
